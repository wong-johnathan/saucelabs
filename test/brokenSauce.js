...
describe("Broken Sauce", function () {
  it("should go to Google and click Sauce", async function () {
    try {
      let driver = await new Builder().withCapabilities(utils.brokenCapabilities).usingServer(ONDEMAND_URL).build();
      ...
      // You can investigate the modal elements using a Live Test(https://app.saucelabs.com/live/web-testing)
        ...
        let search = await driver.findElement(By.className("gLFyf gsfi"));
        await search.sendKeys("Sauce Labs");

        let button = await driver.findElement(By.className("gNO89b"));
        await button.click();

        let page = await driver.findElement(By.partialLinkText("sauce")).click();

      let resourceTab = await driver.findElement(By.linkText("Resources"));
      await driver.actions().move({ origin: resourceTab }).perform();
